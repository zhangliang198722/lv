...
@Repository("PregnancyDao")
public class PregnancyDaoImpl extends BaseHibernateDao<Pregnancy, Integer> implements
		PregnancyDao {
	...
	@Override
	public List<Pregnancy> query(int pn, int pageSize, Pregnancy command) {
		return list(HQL_LIST_QUERY_ALL, pn, pageSize, getQueryParam(command));
	}

	@Override
	public int countQuery(Pregnancy command) {
		return this.<Number> aggregate(HQL_COUNT_QUERY_ALL,
				getQueryParam(command)).intValue();
	}

	private Object[] getQueryParam(Pregnancy command) {
		// TODO 改成全文索引
		String usernameLikeStr = "%" + command.getPregnancyDate() + "%";
