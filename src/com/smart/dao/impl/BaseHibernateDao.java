...
import com.smart.dao.util.ConditionQuery;
import com.smart.dao.util.OrderBy;
import com.smart.page.PageUtil;
import com.smart.util.Assert;
...
public abstract class BaseHibernateDao<M extends java.io.Serializable, PK extends java.io.Serializable>
	extends AbstractDao	implements IBaseDao<M, PK> {
...
//	private JDBCDao jdbcDao;
	...
	@Override
	public List<M> listAll(int start, int pageSize) {
		return list(HQL_LIST_ALL, start, pageSize);
	}
	...
	@SuppressWarnings("unchecked")
	protected <T> List<T> list(final String hql, final int start,
			final int pageSize, final Object... paramlist) {
		Query query = getSession().createQuery(hql);
		setParameters(query, paramlist);
