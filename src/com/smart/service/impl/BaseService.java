...
import com.smart.bean.Constants;
...
public abstract class BaseService<M extends java.io.Serializable, PK extends java.io.Serializable> implements IBaseService<M, PK> {
    ...
    @Override
    public Page<M> listAll(int start, int pageSize) {
        Integer count = countAll();
        List<M> items = baseDao.listAll(start, pageSize);
        return PageUtil.getPage(count, start, items, pageSize);
    }
