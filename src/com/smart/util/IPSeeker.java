...
import com.smart.bean.IPEntry;
import com.smart.bean.IPLocation;
/**
...
public class IPSeeker {
	...
	private IPSeeker()  {
		...
		b4 = new byte[4];
		b3 = new byte[3];
		StringBuilder path = new StringBuilder(Thread.currentThread().getContextClassLoader().getResource("").toString().substring(5));
		path.append("com").append(File.separator).append("ishowchina").append(File.separator).append("data").append(File.separator).append("QQwry.Dat");
		try {
			ipFile = new RandomAccessFile(path.toString(), "r");
	    ...
	    for(long offset = ipBegin + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
	        ...
	        if(temp != -1) {
	            ...
	            if(loc.country.indexOf(s) != -1 || loc.area.indexOf(s) != -1) {
	                ...
	                // 得到起始IP
	    	        readIP(offset - 4, b4);
	                entry.beginIp = IpTools.getIpStringFromBytes(b4);
	                // 得到结束IP
	                readIP(temp, b4);
	                entry.endIp = IpTools.getIpStringFromBytes(b4);
	                // 添加该记录
	                ret.add(entry);
	            }
	        }
	...
	public String getCountry(byte[] ip) {
		...
		// 保存ip，转换ip字节数组为字符串形式
		String ipStr = IpTools.getIpStringFromBytes(ip);
	...
	public String getCountry(String ip) {
	    return getCountry(IpTools.getIpByteArrayFromString(ip));
	}
	/**
	 * 根据IP得到地区名
	 * @param ip ip的字节数组形式
	...
	public String getArea(String ip) {
	    return getArea(IpTools.getIpByteArrayFromString(ip));
	}
	/**
	...
	private String readString(long offset) {
		try {
			...
			if(i != 0)
			    return IpTools.getString(buf, 0, i, "GBK");
		} catch (IOException e) {
		    System.out.println(e.getMessage());
		}
		return "";
	...
	public static void main(String[] args) {
		...
		String s = ipSeeker.getCountry("125.46.14.244");
		int beginIndex = s.indexOf("省");
		int endIndex = s.indexOf("市");
		String city="全国";
		if(endIndex!=-1){
			if(beginIndex==-1){
				city=s.substring(0, endIndex+1);
			}else{
				city=s.substring(beginIndex+1, endIndex+1);
			}
		}
	}
}
