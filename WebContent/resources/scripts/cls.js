...
Cls.form.DateTimeField = Ext.extend(Ext.form.DateField, {
	...
	if (this.getValue()) {
		...
		if(d>new Date())
		{
			//Smart.app.showError("选择时间不可以大于当前时间！");
			return;
		}
        this.setValue(d);
...
//                    }
    ...
    update : function(date, forceRefresh){
        ...
        //修改了时和分的最大时间 this.maxDate ====>new Date()
        var min = this.minDate ? this.minTime(this.minDate).getTime() : Number.NEGATIVE_INFINITY;
        var max = this.maxDate ? this.maxTime(new Date()).getTime() : Number.POSITIVE_INFINITY;
        ...
        for(var i=0; i < this.max; i++) {
        	if(this.header=="分"){
        		if(i%10==0){
    	        	...
    	        	cells[i/10].className = "x-date-active";
    	            setCellClass(this, cells[i/10]);
        		}
        	}else{
	        	this.updateTimeField(d,i);
